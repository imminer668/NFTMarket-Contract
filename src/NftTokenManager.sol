...
contract NftTokenManager is ERC721A, Ownable, AccessControl {
    ...
    constructor(
        ...
        bytes32 _merkleRoot
    ) ERC721A("FishCakeNFT", "FCK") Ownable(msg.sender) {
    ...
    function batchMintRemainingTokens(
        address receiver,
        uint256 amount
    ) external onlyRoleOrAdmin(MINTER_ROLE) {
        if (_totalMinted() + amount > supplyLimit)
            revert("Supply limit reached");
    ...
    function setAllowlistMint(
        bool status
    ) external onlyRoleOrAdmin(PAUSER_ROLE) {
